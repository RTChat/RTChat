		...
		busboy.on('finish', function() {
			...
			if (errorMsg) {
				...
				var tid = setInterval(function() {
					...
					clearInterval( tid );
					splitAndUpload(tmpDir.name, fields, function() {
						//TODO: progress??
						tmpDir.removeCallback(); // Delete all temp files.
...
function splitAndUpload(dir, fields, callback) {
	...
	var access_token = getAccessToken(fields.refresh_token);

	// Create album if it doesn't exist.
	var r = https.request({
		method: 'POST',
		host: 'api.imgur.com',
		path: '/3/album?title='+fields.name,
		headers: {
