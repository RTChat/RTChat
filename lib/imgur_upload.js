...
try {
	...
	// load from './keys/'
	Keys = require(path.join(path.resolve('keys/imgur.json')));
}
...
// 	"client_secret": "xxx",
// }]
// Converts to: { "client_id": "client_secret" }
Keys = Keys.reduce(function(o, i) {o[i.client_id] = i.client_secret; return o;}, {});
...
function getAccessToken(client_id, refresh_token, callback) {
	var data = [
		"grant_type=refresh_token",
		"client_id="+ client_id,
		"client_secret="+ Keys[client_id],
...
function handleUpload(request, response) {
	var requiredFields = ['name', 'username', 'client_id', 'refresh_token'];
		...
		busboy.on('finish', function() {
			...
			if (errorMsg) {
				...
				var tid = setInterval(function() {
					...
					clearInterval( tid );
					getAccessToken(fields.client_id, fields.refresh_token, function(access_token) {
...
function splitAndUpload(dir, fields, access_token, callback) {
	...
	list.forEach(function(fname, i) {
		...
		var file = fs.createReadStream(path.join(dir, fname));
		...
		var req = https.request({
			...
			resp.on('end', function() {
				...
				finishedUploading[i] = JSON.parse(body).data.id;
			});
	...
	var tid = setInterval(function() {
		// If everything is finished uploading
		if(finishedUploading.reduce(function(a,b) {return !!a && !!b;})) {
			clearInterval(tid);
