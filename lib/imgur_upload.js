...
function splitAndUpload(dir, fields, access_token, callback) {
	...
	switch (type) {
		case 'msi', 'zip': // MS format like ppt or doc... //FIXME: For some reason pptx shows up as 'zip'...
			res = spawn('unoconv', ['-f', 'pdf', path.join(dir, 'original_file')]); // convert to pdf
			res = spawn('convert', [path.join(dir, 'original_file.pdf'), path.join(dir, 'i.png')]);
			fs.unlinkSync(path.join(dir, 'original_file.pdf')); //TODO: remove "original_file.*" from upload.
			break;

		case 'pdf':
			res = spawn('convert', [path.join(dir, 'original_file'), path.join(dir, 'i.png')]);
			break;
		default:
